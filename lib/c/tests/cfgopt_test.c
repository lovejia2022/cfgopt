...
int main() {
    ...
    struct cfgopt_parser p = cfgopt_new_parser(argv, sizeof(argv) / sizeof(char const*));
    struct cfgopt_result r;

    r = cfgopt_parse(&p, infos, sizeof(infos) / sizeof(struct cfgopt_flag_info));
...

    {
        ...
        r = cfgopt_args_parse(&cfg, sizeof(args) / sizeof(args[0]), args);
        assert(r.type == CFGOPT_OK);
...

    {
        ...
        char const* args[] = {"foo", "--boolean_array=false", "--boolean_array=true"};
